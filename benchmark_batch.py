...
TEST_TEXT = "I'm seeing a 2021 Honda CR-V on your account - does that look right?"
VOICE = "tara"
MODEL = "orpheus"
SPEED = 1.0
RESPONSE_FORMAT = "wav"
...
def run_single_test(text, voice, model, speed, response_format, save_file=None, session=None):
    ...
    speed : float
        Speech speed.
    response_format : str
        Response format (e.g., "wav").
    save_file : str or None, optional
        If provided, the received audio will be written to this path.
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Save the complete audio file if requested.
            if save_file:
...
def main():
    ...
    with requests.Session() as session:
        ...
        # Warmup run
        print("\nRunning warmup...")
        warmup_result = run_single_test(WARMUP_TEXT, VOICE, MODEL, SPEED, RESPONSE_FORMAT, session=session)
        ...
        for run in range(1, NUM_RUNS + 1):
            ...
            result = run_single_test(TEST_TEXT, VOICE, MODEL, SPEED, RESPONSE_FORMAT, audio_filename, session=session)
